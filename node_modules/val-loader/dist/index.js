...
function execute(code, loaderContext) {
  const module = new _module.default(loaderContext.resource, parentModule);

  // eslint-disable-next-line no-underscore-dangle
  module.paths = _module.default._nodeModulePaths(loaderContext.context);
...
function processResult(loaderContext, result) {
  ...
  (result.contextDependencies || []).forEach(dep => loaderContext.addContextDependency(dep));
  (result.buildDependencies || []).forEach(dep => loaderContext.addBuildDependency(dep));
...
async function loader(content) {
  ...
  if (executableFile) {
    try {
      ...
      try {
        ...
        if (requireError.code === "ERR_REQUIRE_ESM" && _url.pathToFileURL && importESM) {
          const urlForConfig = (0, _url.pathToFileURL)(executableFile);
  ...
  if (typeof func !== "function") {
    callback(new Error(`Module "${this.resource}" does not export a function as default`));
    return;
  }
  let result;
  try {
